...
M1에서 화면 밖으로 나간 오브젝트는 언마운트하세요. 실측은 끝났고(위 표) 상한이 게임 디자인을 못 버티면 ADR-002를 재검토합니다.
...
**토큰 값 변경도 적용됐습니다** (Q10 통합과 함께, ADR-010). 정본은 `app/globals.css` 입니다.

| 토큰 | 이전 | 현재 |
| --- | --- | --- |
| `--c-hazard-500` | `#ff3d6e` | `#c00031` |
| `--c-hazard-700` | `#b81742` | `#6d001c` |
| `--c-hazard-300` | `#ffa8be` | `#ff8fac` |
| `--c-sepia-500` | `#b07a50` | `#b7865f` |

`--c-hazard-700` 은 목표표에 없었지만 **같이 바꿔야 했습니다.** 굵은 외곽선이 이 색인데, 기존 `#b81742` 는 새 본체 `#c00031` 과 대비가 **1.01:1** 이라 테두리가 통째로 사라집니다. 색과 무관한 형태 단서가 목적이므로 본체와 갈리는 값(`#6d001c`, 1.97:1)으로 내렸습니다. `--c-hazard-300` 도 새 본체에 맞춰 한 단계 낮췄습니다 — `character/expressions/eyes-hurt.svg` 가 이 토큰을 참조합니다.

**색각이상 시뮬레이션 결과** (Brettel/Viénot 근사, 위험 파편과의 RGB 거리)

| | 적록색약 | 적색맹 |
| --- | --- | --- |
| cable | 7.9 → **94.5** | 7.3 → **94.1** |
| metal | 42.3 → 87.3 | 25.2 → 86.6 |
| crystal | 69.2 → 159.7 | 69.1 → 161.3 |
| archive | 77.3 → 70.3 | 64.3 → 67.7 |

가장 위험했던 **케이블(7.9 = 사실상 같은 색)이 해소**됐습니다. archive는 적록색약에서 소폭 내려갔지만 70대라 문제 없는 범위입니다.
